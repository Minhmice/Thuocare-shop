    ...
    <section className={cn("bg-white", className)}>
      <div className="mx-auto max-w-[1200px] px-3 py-8 sm:px-4">
        <div className="mb-4 flex items-end justify-between gap-4">
          ...
          <a className="shrink-0 text-sm font-semibold text-[color:var(--lc-blue-700)] hover:underline" href="#">
            {actionLabel}
          </a>
        </div>
        {children}
...
export function LongChauPromoCommerce({
  className,
  ...
  return (
    <section className={cn("bg-[color:var(--lc-surface)]", className)}>
      <div className="mx-auto max-w-[1200px] px-3 py-8 sm:px-4">
        <div className="rounded-3xl border bg-white shadow-sm">
          <div className="grid gap-4 p-5 md:grid-cols-[340px_1fr] md:gap-6 md:p-6">
            <div className="rounded-2xl bg-gradient-to-b from-[color:var(--lc-blue-900)] to-[color:var(--lc-blue-700)] p-5 text-white">
              ...
              <div className="mt-4 rounded-2xl bg-white/12 p-4">
                ...
                <div className="mt-2 flex items-center gap-2 text-sm font-extrabold">
                  <span className="rounded-lg bg-white/15 px-2 py-1">02</span>
                  :
                  <span className="rounded-lg bg-white/15 px-2 py-1">14</span>
                  :
              ...
              <Button
                className="mt-5 h-10 w-full rounded-full bg-white text-[color:var(--lc-blue-900)] hover:bg-white/90"
              >
                Mua ngay
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {products.slice(0, 6).map((p) => (
                <LongChauProductCardV2 key={p.id} product={p} variant="compact" />
              ))}
            </div>
          </div>
        </div>
...
export function LongChauBestSellers({
  className,
  products,
}: {
  className?: string;
    ...
    <SectionShell title="Sản phẩm bán chạy" description="So sánh nhanh theo công dụng, dạng bào chế và quy cách." className={className}>
      <div className="mb-4 flex flex-wrap gap-2">
        {chips.map((t) => (
          <button
            key={t}
            type="button"
            className="rounded-full border bg-white px-3 py-1.5 text-sm font-semibold text-slate-700 shadow-sm hover:border-[color:var(--lc-blue-700)] hover:text-[color:var(--lc-blue-700)]"
          >
            {t}
          </button>
        ))}
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {products.slice(0, 12).map((p) => (
          <LongChauProductCardV2 key={p.id} product={p} />
        ))}
      </div>
    </SectionShell>
  );
...
export function LongChauBrandStrip({ className }: { className?: string }) {
...
export function LongChauSeasonalSolutions({
  className,
  products,
}: {
  className?: string;
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 lg:grid-cols-[420px_1fr]">
        <div className="rounded-3xl border bg-[color:var(--lc-surface)] p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-2 text-sm text-slate-800">
            <div className="flex items-start gap-2">
              <span className="mt-0.5 inline-block size-2 rounded-full bg-[color:var(--lc-blue-700)]" />
              <div>
                <div className="font-semibold">Theo dõi dấu hiệu nặng</div>
                <div className="text-sm text-slate-600">Sốt cao, lừ đừ, đau ngực, phát ban lan nhanh…</div>
              </div>
            </div>
            <div className="flex items-start gap-2">
              <span className="mt-0.5 inline-block size-2 rounded-full bg-[color:var(--lc-blue-700)]" />
              <div>
                <div className="font-semibold">Chăm sóc tại nhà đúng cách</div>
                <div className="text-sm text-slate-600">Uống đủ nước, nghỉ ngơi, vệ sinh mũi họng…</div>
              </div>
            </div>
            <div className="flex items-start gap-2">
              <span className="mt-0.5 inline-block size-2 rounded-full bg-[color:var(--lc-blue-700)]" />
              <div>
                <div className="font-semibold">Hỏi dược sĩ khi cần</div>
                <div className="text-sm text-slate-600">Chọn đúng sản phẩm, đúng đối tượng, đúng liều dùng.</div>
              </div>
            </div>
          </div>

          <div className="mt-5 flex flex-wrap gap-2">
            <Button className="h-9 rounded-full bg-[color:var(--lc-blue-700)] text-white hover:bg-[color:var(--lc-blue-800)]">
              Hỏi dược sĩ
              <ArrowRight className="size-4" />
            </Button>
...
export function LongChauVideoRail({ className }: { className?: string }) {
  const featured = {
    title: "Dược sĩ tư vấn: 4 lưu ý khi chăm sóc sức khỏe mùa nắng nóng",
    duration: "06:32",
  };
  const items = [
    ...
    <SectionShell title="Video sức khỏe" description="Nội dung ngắn gọn, dễ hiểu, ưu tiên tính y khoa và ứng dụng." className={className}>
      <div className="grid gap-3 lg:grid-cols-[1.2fr_0.8fr]">
        <a href="#" className="group rounded-3xl border bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow">
          <div className="relative overflow-hidden rounded-2xl bg-[color:var(--lc-surface)]">
        ...
        <div className="grid gap-3">
          {items.map((v) => (
            <a
              key={v.title}
              href="#"
              className="flex gap-3 rounded-3xl border bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow"
            >
              <div className="relative w-36 overflow-hidden rounded-2xl bg-[color:var(--lc-surface)]">
                <div className="aspect-video w-full" />
                <div className="absolute bottom-2 left-2 rounded-full bg-black/55 px-2 py-0.5 text-[11px] font-semibold text-white">
                  {v.duration}
                </div>
              </div>
              <div className="min-w-0">
...
export function LongChauArticleFeatureSection({
  className,
  featuredArticle,
}: {
  className?: string;
  featuredArticle: DbArticle | null;
}) {
    ...
    <SectionShell title="Bài viết & Tin sức khỏe" description="Nội dung đáng tin cậy — giúp bạn quyết định mua đúng và chăm sóc đúng." className={className}>
      <div className="grid gap-3 lg:grid-cols-[1.1fr_0.9fr]">
        <a href={featured.href} className="rounded-3xl border bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow">
          <div className="overflow-hidden rounded-2xl bg-[color:var(--lc-surface)]">
            <div className="aspect-[16/9] w-full" />
          </div>
          <div className="mt-3 flex items-center gap-2 text-xs font-semibold text-slate-600">
            <Newspaper className="size-4 text-[color:var(--lc-blue-700)]" />
            {featured.category}
            <span className="text-slate-400">•</span>
            {featured.meta}
          </div>
          <div className="mt-2 text-base font-extrabold text-slate-900">{featured.title}</div>
          <div className="mt-2 inline-flex items-center gap-1 text-sm font-semibold text-[color:var(--lc-blue-700)]">
            Đọc bài
            <ArrowRight className="size-4" />
          </div>
        </a>
        ...
        <div className="grid gap-2">
          {items.map((a) => (
            <a
              key={a.title}
              href="#"
              className="flex items-start gap-3 rounded-3xl border bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow"
            >
              <div className="mt-0.5 size-16 shrink-0 overflow-hidden rounded-2xl bg-[color:var(--lc-surface)]" />
              <div className="min-w-0">
...
export function LongChauHealthSegments({ className }: { className?: string }) {
  const segments = [
    {
      title: "Sức khỏe Nam giới",
      items: ["Sinh lý nam", "Tim mạch", "Gan mật", "Xương khớp"],
    },
    {
      title: "Sức khỏe Nữ giới",
      items: ["Nội tiết", "Da liễu", "Tiền mãn kinh", "Sức khỏe sinh sản"],
    },
    {
      title: "Chăm sóc Người cao tuổi",
      items: ["Trí nhớ", "Huyết áp", "Xương khớp", "Tiểu đường"],
    },
    {
      title: "Chăm sóc Trẻ em",
      items: ["Miễn dịch", "Dinh dưỡng", "Hô hấp", "Da liễu"],
    },
  ];
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {segments.map((s) => (
          <div key={s.title} className="rounded-3xl border bg-white p-5 shadow-sm">
            ...
            <a
              href="#"
              className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-[color:var(--lc-blue-700)] hover:underline"
            >
              Khám phá
              <ArrowRight className="size-4" />
            </a>
...
export function LongChauTrustBand({ className }: { className?: string }) {
  const items = [
  ...
  return (
    <section className={cn("bg-white", className)}>
      <div className="mx-auto max-w-[1200px] px-3 py-8 sm:px-4">
        <div className="grid gap-3 rounded-3xl border bg-[color:var(--lc-surface)] p-5 shadow-sm sm:grid-cols-2 lg:grid-cols-6">
          {items.map((it) => (
            <div key={it.title} className="flex items-start gap-3 rounded-2xl bg-white p-4">
              <div className="grid size-10 shrink-0 place-items-center rounded-2xl bg-[color:var(--lc-surface)]">
                <it.icon className="size-5 text-[color:var(--lc-blue-700)]" />
              </div>
              <div className="min-w-0">
...
export const demoProducts: ProductCard[] = [
  ...
  },
  {
    id: "p4",
    brand: "Panadol",
    title: "Panadol giảm đau hạ sốt cho người lớn",
    originLabel: "Anh",
    benefitTag: "Giảm đau, hạ sốt",
    formatTag: "Viên nén",
    packNote: "Hộp 12 viên",
    trustLabel: "Hỗ trợ đơn thuốc",
    isAuthenticitySupported: true,
    badge: "Thiết yếu",
    imageUrl:
      "https://cdn.nhathuoclongchau.com.vn/unsafe/256x256/filters:quality(90):format(webp)/products/2024/02/00000000.png",
    price: { amount: 35000, display: "35.000đ" },
    compareAt: { amount: 42000, display: "42.000đ" },
    unit: "Hộp",
  },
  {
    id: "p5",
    brand: "Nature’s Way",
    title: "Kẹo dẻo vitamin cho trẻ em (Vị trái cây)",
    originLabel: "Úc",
    benefitTag: "Cho trẻ em",
    formatTag: "Kẹo dẻo",
    packNote: "Hũ 60 viên",
    trustLabel: "Tư vấn dược sĩ",
    isAuthenticitySupported: true,
    ...
    imageUrl:
      "https://cdn.nhathuoclongchau.com.vn/unsafe/256x256/filters:quality(90):format(webp)/products/2024/02/00000000.png",
    price: { amount: 315000, display: "315.000đ" },
    compareAt: { amount: 379000, display: "379.000đ" },
    unit: "Hũ",
  },
  {
    id: "p6",
    brand: "Omron",
    title: "Máy đo huyết áp điện tử bắp tay",
    originLabel: "Nhật",
    benefitTag: "Huyết áp",
    formatTag: "Thiết bị y tế",
    packNote: "Bảo hành 24 tháng",
    trustLabel: "Chính hãng",
    isAuthenticitySupported: true,
    badge: "Giảm sâu",
    imageUrl:
      "https://cdn.nhathuoclongchau.com.vn/unsafe/256x256/filters:quality(90):format(webp)/products/2024/02/00000000.png",
    price: { amount: 990000, display: "990.000đ" },
    compareAt: { amount: 1290000, display: "1.290.000đ" },
    unit: "Bộ",
  },
];
