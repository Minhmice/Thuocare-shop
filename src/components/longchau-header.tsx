...
import { LONGCHAU_TOP_NAV } from "@/components/longchau-nav.data";
import { LongChauNavPanelView } from "@/components/longchau-nav-panel";
import { Search } from "lucide-react";

export function LongChauHeader({
  className,
  nav = LONGCHAU_TOP_NAV,
}: {
  className?: string;
  nav?: LongChauTopNavItem[];
}) {
  ...
  const activeTop = React.useMemo(
    () => nav.find((i) => i.id === activeTopId) ?? null,
    [activeTopId, nav]
  );
  ...
  return (
    <header className={cn("sticky top-0 z-40 w-full", className)}>
      {/* Main header (search-led) */}
      <div className="bg-white">
        <div className="mx-auto max-w-[1200px] px-3 py-3 sm:px-4">
          <div className="flex items-center gap-3">
            <Link href="/" className="flex shrink-0 items-center gap-2">
              <div className="grid size-10 place-items-center rounded-2xl bg-[color:var(--lc-blue-700)] text-white shadow-sm">
            ...
            <div className="relative hidden flex-1 sm:block">
              ...
              <input
                className="h-11 w-full rounded-full bg-[color:var(--lc-surface)] px-10 pr-28 text-sm text-slate-900 placeholder:text-slate-500 shadow-sm outline-none ring-1 ring-black/5 focus:ring-2 focus:ring-[color:var(--lc-blue-700)]/30"
                placeholder="Tìm thuốc, triệu chứng, hoạt chất, danh mục…"
              />
              <div className="absolute right-2 top-1/2 -translate-y-1/2">
                <Button className="h-9 rounded-full bg-[color:var(--lc-blue-700)] px-5 text-white hover:bg-[color:var(--lc-blue-800)]">
                  Tìm kiếm
                </Button>
              </div>
            </div>
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <Button className="h-10 rounded-full bg-[color:var(--lc-blue-700)] text-white hover:bg-[color:var(--lc-blue-800)]">
                Giỏ hàng
              </Button>
            </div>
          </div>
              ...
              {nav.map((item) => {
                ...
                if (!hasDropdown) {
                  return (
                    <a
                      key={item.id}
                      href={item.href ?? "#"}
                      className="inline-flex items-center gap-1 whitespace-nowrap hover:text-[color:var(--lc-blue-800)]"
                    >
                      {item.label}
                    </a>
                  );
                ...
                return (
                  <button
                    ...
                    type="button"
                    className={cn(
                      "inline-flex items-center gap-1 whitespace-nowrap transition hover:text-[color:var(--lc-blue-800)]",
                      isActive && "text-[color:var(--lc-blue-800)]"
                    )}
                    aria-haspopup="menu"
                    ...
                    onClick={() => {
                      // Mobile tap: toggle. Desktop hover: click is harmless.
                      setActiveTopId((cur) => (cur === item.id ? null : item.id));
                    }}
                  >
                    {item.label}
              ...
              <>
                ...
                <div className="pointer-events-none absolute left-0 right-0 top-full hidden sm:block">
                  <div className="pointer-events-auto mx-auto max-w-[1200px] px-3 pb-4 pt-3 sm:px-4">
                    <LongChauNavPanelView
                      panel={activePanel}
                      mode="desktop"
                      activeSidebarId={activeSidebarId}
                      setActiveSidebarId={setActiveSidebarId}
                    />
                  </div>
                </div>
