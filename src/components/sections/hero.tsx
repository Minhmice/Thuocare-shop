...
export function Hero({ className }: { className?: string }) {
  ...
  return (
    <section className={cn("bg-(--lc-blue-950)/5", className)}>
      <div className="mx-auto max-w-[1200px] px-3 py-3 sm:px-4 sm:py-4">
        <div className="relative overflow-hidden rounded-2xl bg-(--lc-blue-900)">
          <Image
            src={slide.imageUrl}
            alt={slide.alt}
            width={1200}
          ...
          <div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 items-center gap-2">
            {SLIDES.map((s, idx) => (
              <button
                ...
                className={cn("h-2 w-2 rounded-full bg-white/60", idx === active && "w-6 bg-white")}
              />
            ))}
          </div>
          ...
          <div className="absolute bottom-4 left-4">
            <Button className="pointer-events-auto h-9 rounded-full bg-white px-6 text-(--lc-blue-800) hover:bg-white/90">
              {slide.ctaLabel}
            </Button>
          </div>
        </div>
